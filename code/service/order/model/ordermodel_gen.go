...
var (
	...
	orderRowsExpectAutoSet   = strings.Join(stringx.Remove(orderFieldNames, "`id`", "`update_time`", "`create_at`", "`created_at`", "`create_time`", "`update_at`", "`updated_at`"), ",")
	orderRowsWithPlaceHolder = strings.Join(stringx.Remove(orderFieldNames, "`id`", "`update_time`", "`create_at`", "`created_at`", "`create_time`", "`update_at`", "`updated_at`"), "=?,") + "=?"
...
type (
	...
	Order struct {
		...
		Status     int64     `db:"status"` // 订单状态，0表示未支付，1表示已支付，默认为0
...
func (m *defaultOrderModel) Insert(ctx context.Context, data *Order) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.Uid, data.Fid, data.Amount, data.Status)
	}, orderIdKey)
	return ret, err
}
...
func (m *defaultOrderModel) Update(ctx context.Context, data *Order) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.Uid, data.Fid, data.Amount, data.Status, data.Id)
	}, orderIdKey)
	return err
}
