...
var (
	...
	payRowsExpectAutoSet   = strings.Join(stringx.Remove(payFieldNames, "`id`", "`update_at`", "`updated_at`", "`update_time`", "`create_at`", "`created_at`", "`create_time`"), ",")
	payRowsWithPlaceHolder = strings.Join(stringx.Remove(payFieldNames, "`id`", "`update_at`", "`updated_at`", "`update_time`", "`create_at`", "`created_at`", "`create_time`"), "=?,") + "=?"
...
type (
	...
	Pay struct {
		...
		Source     int64     `db:"source"` // 支付方式，0表示线下支付，1表示线上支付，默认为线下支付
		Status     int64     `db:"status"` // 支付状态，0表示未支付，1表示已支付，默认为0
